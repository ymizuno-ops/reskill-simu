...
**🔒 免責事項**
- 本アプリの利用により生じた損害・不利益（転職の失敗、収入の減少、その他直接的・間接的損害を含む）について、開発者は一切の責任を負いません。
- 本アプリが提供する情報は参考目的に限定されるものであり、開発者はその正確性・完全性・最新性を保証しません。
- ユーザーが本アプリの情報をもとに行った判断・行動の結果について、開発者は責任を負いません。
...
def main() -> None:
    ...
    if not sim_done:
        ...
        with st.expander("📊 スキル引継ぎ率ガイド", expanded=guide_expanded):
            st.markdown(
                "転職先でどの程度スキルが評価されるかを設定します。初年度年収の計算に使用されます。"
            )
            st.dataframe(
                pd.DataFrame(skill_transfer_data),
                use_container_width=True,
                hide_index=True,
            )
            st.caption(
                "※ シミュレーション実行後は目標職種・年齢・年収をもとに実際の初年度年収も表示されます。"
            )
