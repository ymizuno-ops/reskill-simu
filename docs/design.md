...
リファクタリングにより、単一の `main.py`（旧 `app.py`）から以下のモジュールに分割済み。
...
```python
st.session_state["disclaimer_accepted"] = bool  # 免責事項同意フラグ
...
{
  "latest_year":            2024,
  "avg_wage_growth_10yr":   0.0055,
  "avg_gdp_growth_10yr":    0.0056,
  "avg_cpi_10yr":           0.0123,
  "real_wage_growth_10yr":  -0.0068,
  "forecast_nominal_raise": 0.0055,
  "forecast_real_raise":    0.0
}
```
...
| plotly | >=5.0.0 | インタラクティブグラフ描画 |
| openpyxl | >=3.1.0 | xlsx読み込み |
| protobuf | >=4.25.0 | Streamlit依存 |
| blinker | >=1.7.0 | Streamlit依存 |
| lightgbm | >=4.0.0 | LightGBMモデル |
| catboost | >=1.2.0 | CatBoostモデル |
| xgboost | >=2.0.0 | XGBoostモデル |
