...
## Phase 4: Streamlit アプリ基礎

### Task 19: ページ設定・CSS の実装（main.py）
...
# グラフは Plotly を使用するため日本語フォント設定は不要
```

---

### Task 20: アセット読み込み関数の実装（main.py）
...
### Task 21: Wrapper クラスのインポート（pickle復元用）（main.py）
...
### Task 22: predict() 関数の実装（simulation.py）
...
### Task 23: サイドバーの実装（ui/sidebar.py）

```python
def render_sidebar(occ_list, models, macro) -> tuple:
    # ── プロフィール設定 ──
    ...
    # tgt_cat → tgt_occs → target_occ（occupation.py の build_category_occ_map を使用）
...
**職種カテゴリ（15カテゴリ）の定義**: `occupation.py` の `OCCUPATION_CATEGORIES` を参照

---

### Task 24: セッション状態管理・実行ボタンの実装（main.py）
...
### Task 30: 年収推移グラフ（2軸）の実装（ui/charts.py）

```python
def plot_main_plotly(sq, cc, current_age, current_occ, target_occ, cost) -> go.Figure:
    # trace1: 年収推移（現状=青, 転職後=赤）
    # trace2: 累積収支差額（緑点線、secondary_y=True）
    # 学習コスト水平線: orange 破線
    # Plotly を使用するため日本語フォント設定は不要
```

---

### Task 31: 全モデル比較グラフの実装（ui/charts.py）

```python
def plot_all_models_plotly(sq_all, cc_all, current_age, breakevens, model_labels) -> go.Figure:
    # make_subplots でモデル数に応じた動的サブプロット
...
確認事項:
  - Plotly グラフのタイトル・軸ラベル・凡例が文字化けしないこと
  - Plotly はブラウザレンダリングのため日本語フォント設定は原則不要
```

### Task 38: README.md の作成

```
記載内容:
  - 概要・ターゲットユーザー
  - ディレクトリ構成（main.py / occupation.py / simulation.py / ui/ を含む）
  - セットアップ手順（6ステップ）
  - 搭載モデル一覧
  - シミュレーションロジック説明
  - データソース一覧
...
□ streamlit run main.py でブラウザが開くこと
